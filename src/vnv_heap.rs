...
impl<A: AllocatorModule, R: PageReplacementModule, S: PageStorageModule> VNVHeapInner<A, R, S> {
    ...
    pub(crate) unsafe fn deallocate<T>(&mut self, layout: &Layout, identifier: &AllocationIdentifier<T, A>) {
    ...
    pub(crate) unsafe fn get_ref<T>(&mut self, identifier: &AllocationIdentifier<T, A>) -> *const T{
