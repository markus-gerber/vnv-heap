...
use bench::{
    calc_obj_cnt_and_rem_size_max_dirty, calc_obj_cnt_and_rem_size_max_objects,
    PersistLatencyBenchmark,
};

use super::*;
use crate::{
    benchmarks::{BenchmarkRunOptions, BenchmarkRunner, RunAllBenchmarkOptions, Timer}, calc_resident_buf_cutoff_size, modules::persistent_storage::DummyStorageModule, resident_object_manager::resident_object_metadata::ResidentObjectMetadata, util::round_up_to_nearest, VNVObject
};
...
const MAX_DIRTY_SIZE: usize = 2 * 1024 - VNV_HEAP_RAM_OVERHEAD;
    ...
    >(
        ...
        if options.run_buffer_size_persist_latency {
            ...
            for_buffer_size!(BUF_SIZE, {
                ...
                const RES: (usize, usize, usize, bool) = calc_obj_cnt_and_rem_size_max_objects(
                    REM_DIRTY_SIZE,
                    BUF_SIZE - RESIDENT_CUTOFF_SIZE,
                );
                ...
                let bench: PersistLatencyBenchmark<BUF_SIZE, RESIDENT_CUTOFF_SIZE, REM_OBJ_SIZE, VNV_HEAP_RAM_OVERHEAD> =
                    PersistLatencyBenchmark::new::<S>(
                        DIRTY_SIZE,
                        &mut heap,
                        OBJ_CNT,
                        DIRTY_NORMAL_OBJECTS,
                        REM_OBJ_DIRTY,
                        "max_objects_persist_latency_buffer_size",
                    );
            ...
            for_buffer_size!(BUF_SIZE, {
                ...
                const RES: (usize, usize, usize, bool) = calc_obj_cnt_and_rem_size_max_dirty(
                    REM_DIRTY_SIZE,
                    BUF_SIZE - RESIDENT_CUTOFF_SIZE,
                );
                const OBJ_CNT: usize = RES.0;
                ...
                let bench: PersistLatencyBenchmark<BUF_SIZE, RESIDENT_CUTOFF_SIZE, REM_OBJ_SIZE, VNV_HEAP_RAM_OVERHEAD> =
                    PersistLatencyBenchmark::new::<S>(
                        ...
                        DIRTY_NORMAL_OBJECTS,
                        REM_OBJ_DIRTY,
                        "max_dirty_persist_latency_buffer_size",
                    );
                bench.run_benchmark::<TIMER, TRIGGER>(run_options, get_ticks, &mut trigger);
            });
        }
