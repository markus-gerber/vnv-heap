...
use crate::modules::allocator::linked_list::internal::align_down_size;

use super::internal::{align_up, align_up_size};
...
impl Cursor {
    ...
    unsafe fn split_at(self, required_layout: Layout, ptr: *mut u8) -> Result<(), Self> {
        ...
        // live to the point where we start doing pointer surgery below.
        {
            ...
            if (ptr as usize) > hole_addr_u8 as usize + hole_size {
                return Err(self);
            }

            if (ptr as usize) < hole_addr_u8 as usize {
                return Err(self);
            }

            // Attempt to fracture the current hole into the following parts:
            // ([front_padding], allocation, [back_padding])
            let aligned_addr = if hole_addr_u8 == ptr {
                ...
                let mut aligned_addr = align_up(new_start, required_align);
                if (aligned_addr as usize) < ptr as usize {
                    aligned_addr = ptr;
                    assert_eq!(ptr, align_up(ptr, required_align), "ptr has to be aligned");
                }
...
impl HoleList {
    ...
    // ADDED FUNCTION FOR VNV HEAP
    pub(crate) unsafe fn allocate_at(&mut self, layout: Layout, ptr: *mut u8) -> Result<Layout, ()> {
        let aligned_layout = Self::align_layout(layout);
        let ptr = ptr as usize;
        let end_ptr = ptr + aligned_layout.size();
        let mut cursor = self.cursor().ok_or(())?;

        loop {
            let curr = cursor.hole.as_ptr() as usize;
            if curr > ptr {
                // hole not found, cancel search
                return Err(());
            }
            
            let end_ptr_curr = curr + cursor.current().size;
            if end_ptr > end_ptr_curr {
                // hole is too small to allocate the layout
                // continue search
                cursor = cursor.next().ok_or(())?;
            } else {
                // hole found
                // split here
                match cursor.split_at(aligned_layout.clone(), ptr as *mut u8) {
                    Ok(()) => {
                        return Ok(aligned_layout);
                    }
                    Err(curs) => {
...
#[cfg(test)]
pub(super) mod test {
    use crate::modules::allocator::linked_list::hole::Hole;
    ...
    pub(crate) fn check_cursor_integrity(mut cursor1: Option<Cursor>, mut cursor2: Option<Cursor>, difference: isize) {
        loop {
            match (cursor1, cursor2) {
                (Some(cur1), Some(cur2)) => {
                    let ptr1 = (cur1.current() as *const Hole) as usize;
